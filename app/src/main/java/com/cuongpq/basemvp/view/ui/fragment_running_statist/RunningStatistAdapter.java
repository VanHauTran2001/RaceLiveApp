...
import com.cuongpq.basemvp.databinding.ItemStatistRunningBinding;
import com.cuongpq.basemvp.model.Car;

public class RunningStatistAdapter extends RecyclerView.Adapter<RunningStatistAdapter.RunningStatistViewHolder>{
    private IRunningStatist iRunningStatist;
    private boolean showInfo = true;

    public RunningStatistAdapter(IRunningStatist iRunningStatist) {
        this.iRunningStatist = iRunningStatist;
    }

    @NonNull
    @Override
    public RunningStatistViewHolder onCreateViewHolder(@NonNull ViewGroup parent, int viewType) {
        ItemStatistRunningBinding binding = ItemStatistRunningBinding.inflate(LayoutInflater.from(parent.getContext()),parent,false);
        return new RunningStatistViewHolder(binding);
    }

    @Override
    public void onBindViewHolder(@NonNull RunningStatistViewHolder holder, int position) {
        Car car = iRunningStatist.getCar(position);
    ...
    public int getItemCount() {
        return iRunningStatist.getCount();
    }

    public interface IRunningStatist{
        int getCount();
        Car getCar(int position);
    }
    public static class RunningStatistViewHolder extends RecyclerView.ViewHolder{
        ItemStatistRunningBinding binding;
        public RunningStatistViewHolder(@NonNull ItemStatistRunningBinding binding) {
